...
public class AndroidNotification extends UmengNotification {
    ...
    public boolean setPredefinedKeyValue(String key, Object value) throws Exception {
        if (ROOT_KEYS.contains(key)) {
            ...
            JSONObject payloadJson = JSONObjectUtil.getObjectCondition(rootJson, payload);
            JSONObject bodyJson = JSONObjectUtil.getObjectCondition(payloadJson, body);
    ...
    public boolean setExtraField(String key, String value) {
        JSONObject payloadJson = JSONObjectUtil.getObjectCondition(rootJson, payload);
        JSONObject extraJson = JSONObjectUtil.getObjectCondition(payloadJson, extra);
        /*JSONObject payloadJson = null;
     ...
     * @since 2019/3/15 11:09
     */
    public void getJsonParam(Map<String, Object> paramMap) {
        if (!paramMap.isEmpty()) {
            for (Map.Entry<String, Object> map : paramMap.entrySet()) {
                try {
                    setPredefinedKeyValue(map.getKey(), map.getValue().toString());
                } catch (Exception e) {
                    log.info("组装数据异常：" + e.getMessage());
                }
            }
        }
    }
     ...
     */
    public void setAndroidBroadcastParam(UmengMessageDTO umengMessageDTO, AndroidBroadcast broadcast) throws Exception {
        setCustomFieldParam(umengMessageDTO, broadcast);
    }
     ...
     */
    public void setAndroidCustomizedCastParam(UmengMessageDTO umengMessageDTO, AndroidCustomizedcast customizedcast) throws Exception {
        customizedcast.setAlias(umengMessageDTO.getAlias(), MessageConstants.ALIAS_TYPE);
        customizedcast.setCustomField("");
        //自定义参数
        setCustomFieldParam(umengMessageDTO, customizedcast);
    }
     ...
     * @since 2019/3/15 14:27
     */
    public void setAndroidUnicastParam(UmengMessageDTO umengMessageDTO, AndroidUnicast unicast) throws Exception {
        unicast.setDeviceToken(umengMessageDTO.getDeviceToken());
        //自定义参数
        setCustomFieldParam(umengMessageDTO, unicast);
    }
     ...
     *
     * @param umengMessageDTO     消息对象
     * @param androidNotification android推送对象
     * @author zhangxiaogang
     * @since 2019/3/15 14:27
     */
    private void setCustomFieldParam(UmengMessageDTO umengMessageDTO, AndroidNotification androidNotification) throws Exception {
        androidNotification.setTicker(umengMessageDTO.getSubTitle());
        androidNotification.setTitle(umengMessageDTO.getTitle());
        androidNotification.setText(umengMessageDTO.getText());
        androidNotification.goCustomAfterOpen("");
        ...
        androidNotification.setCustomField(GsonUtils.format(extraField));
        /*if (!extraField.isEmpty()) {
            for (Map.Entry<String, String> map : extraField.entrySet()) {
                androidNotification.setExtraField(map.getKey(), map.getValue());
            }
        }*/
    }
