 ...
 */
@Controller
@RequestMapping("/usersgroupinfo")
public class UsersGroupInfoController extends BaseWebController {
    @Autowired
    UsersGroupInfoServiceImpl service;
    ...
    public String editEntrance(Model model,@PathVariable("iframe") String iframe,String id) {
        ...
        if(id!=null){
            UsersGroupInfo entity = service.getById(id);
    ...
    public Mono<ResponseEntity<SystemResponse<Object>>> listByPage(@RequestBody SearchElementGrid search) {
        ...
        LambdaQueryWrapper<UsersGroupInfo> ew = wrapper(search);
        ...
        return success(result);
         ...
         /**
     * 添加或修改
     * @param entity 人员分组关系表实体
     * @return
     */
    @PostMapping("/save")
    @ResponseBody
    public Mono<ResponseEntity<SystemResponse<Object>>>  save(@RequestBody UsersGroupInfo entity) {
     ...
     * @return
     */
    private LambdaQueryWrapper<UsersGroupInfo> wrapper(SearchElementGrid search){
        LambdaQueryWrapper<UsersGroupInfo> ew = null;
        if(search!=null){
            ...
            ew = search.toEntityLambdaWrapper(UsersGroupInfo.class);

        }
        else{
            ew = new LambdaQueryWrapper<UsersGroupInfo>();
        }

        ew.orderBy(true, false,UsersGroupInfo::getSort);
     ...
     * @return
     */
    private List<Map<String, Object>> convert2List(List<UsersGroupInfo> list){
       
        return JsonUtil.<UsersGroupInfo>ObjectToList(list,
                new String[]{ "id","idMeGroupInfo","sort","notes","rbacUserId" }, 
                (m, entity) -> {
     ...
     * @return
     */
    private Map<String, Object> convert2Map(UsersGroupInfo ent){
        return JsonUtil.<UsersGroupInfo>ObjectToMap(ent,
                new String[]{ "id","idMeGroupInfo","sort","notes","rbacUserId" }, 
                (m, entity) -> {
                    adapterField(m,entity);
                }
     ...
     */
    private void adapterField(Map<String, Object> m,UsersGroupInfo entity){
    ...
    public Mono<ResponseEntity<SystemResponse<Object>>> changeOrder(@PathVariable Integer id,@PathVariable Integer up){
        ...
        LambdaQueryWrapper<UsersGroupInfo> wrapper = new LambdaQueryWrapper();

        String msg ="";
        if(up==1) {
            wrapper.lt(UsersGroupInfo::getSort, sort);
            msg ="已经是最后一条数据";
            wrapper.orderByDesc(UsersGroupInfo::getSort);
        }
        else {
            wrapper.gt(UsersGroupInfo::getSort, sort);
            msg ="已经是第一条数据";
            wrapper.orderByAsc(UsersGroupInfo::getSort);
        }


        UsersGroupInfo entity1 = service.getOne(wrapper);
        if(entity1==null) throw new UnityRuntimeException(msg);

        entity.setSort(entity1.getSort());

        UsersGroupInfo entityA = new UsersGroupInfo();
        entityA.setId(entity.getId());
        entityA.setSort(entity1.getSort());
        service.updateById(entityA);

        UsersGroupInfo entityB = new UsersGroupInfo();
