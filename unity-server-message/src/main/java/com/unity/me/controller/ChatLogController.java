...
import com.unity.me.enums.MsgTypeEnum;
import com.unity.me.enums.WayEnum;
import com.unity.me.service.ChatLogServiceImpl;
...
public class ChatLogController extends BaseWebController {
    @Autowired
    ChatLogServiceImpl service;
    ...
    public String editEntrance(Model model,@PathVariable("iframe") String iframe,String id) {
        ...
        if(id!=null){
            ChatLog entity = service.getById(id);
    ...
    public Mono<ResponseEntity<SystemResponse<Object>>> listByPage(@RequestBody SearchElementGrid search) {
        ...
        LambdaQueryWrapper<ChatLog> ew = wrapper(search);
        ...
        return success(result);
         ...
         /**
     * 添加或修改
     * @param entity 聊天记录信息实体
     * @return
     */
    @PostMapping("/save")
    @ResponseBody
    public Mono<ResponseEntity<SystemResponse<Object>>>  save(@RequestBody ChatLog entity) {
     ...
     * @return
     */
    private LambdaQueryWrapper<ChatLog> wrapper(SearchElementGrid search){
        LambdaQueryWrapper<ChatLog> ew = null;
        if(search!=null){
            ...
            ew = search.toEntityLambdaWrapper(ChatLog.class);

        }
        else{
            ew = new LambdaQueryWrapper<ChatLog>();
        }

        ew.orderBy(true, false,ChatLog::getSort);
     ...
     * @return
     */
    private List<Map<String, Object>> convert2List(List<ChatLog> list){
       
        return JsonUtil.<ChatLog>ObjectToList(list,
                new String[]{ "id","sort","notes","sender","receiver","content","msgType","way" }, 
                (m, entity) -> {
     ...
     * @return
     */
    private Map<String, Object> convert2Map(ChatLog ent){
        return JsonUtil.<ChatLog>ObjectToMap(ent,
                new String[]{ "id","sort","notes","sender","receiver","content","msgType","way" }, 
                (m, entity) -> {
                    adapterField(m,entity);
                }
     ...
     */
    private void adapterField(Map<String, Object> m,ChatLog entity){
        ...
        m.put("msgTypeTitle", MsgTypeEnum.of(entity.getMsgType()).getName());
        m.put("isReadTitle", FlagEnum.of(entity.getIsRead()).getName());
        m.put("wayTitle", WayEnum.of(entity.getWay()).getName());
    }
    ...
    public Mono<ResponseEntity<SystemResponse<Object>>> changeOrder(@PathVariable Integer id,@PathVariable Integer up){
        ...
        String msg ="";
        if(up==1) {
            wrapper.lt(ChatLog::getSort, sort);
            msg ="已经是最后一条数据";
            wrapper.orderByDesc(ChatLog::getSort);
        }
        else {
            wrapper.gt(ChatLog::getSort, sort);
            msg ="已经是第一条数据";
            wrapper.orderByAsc(ChatLog::getSort);
        }


        ChatLog entity1 = service.getOne(wrapper);
        if(entity1==null) throw new UnityRuntimeException(msg);

        entity.setSort(entity1.getSort());

        ChatLog entityA = new ChatLog();
        entityA.setId(entity.getId());
        entityA.setSort(entity1.getSort());
        service.updateById(entityA);

        ChatLog entityB = new ChatLog();
